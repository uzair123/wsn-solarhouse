...
static void recv_from_unicast(struct unicast_conn *unicast,
	...
	//	PRINTF("[%u.%u:%u] frag_unicast::recv_from_unicast() : From [%u.%u] \n",
	...
	PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : From [%u.%u], last_received_packet_id: %u, received_packet_id: %u, fragment_nr: %u, num_fragments: %u\n",
			rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time(),
			from->u8[1], from->u8[0],
			c->last_received_packet_id, received_packet_id, fragment_nr, num_fragments);

#if DEBUG>=3
	PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Status is [",
			rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time());
	switch (c->status) {
		case (IDLE): {
			PRINTF(3,"IDLE");
	...
	if ((c->status == IDLE) || // either this node was idle, or
		...
		PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Status is correct\n",
				rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time());

		if (fragment_nr == 0) {

			rimeaddr_copy(&c->from, from);
			//			PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : c->from is now [%u.%u]\n",
			//					rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), c->from.u8[1], c->from.u8[0]);
			//			PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Before bitvector_clear, NULL timer is expired: %u\n",
			...
			//			PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : After bitvector_clear, NULL timer is expired: %u\n",
			//					rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), ctimer_expired(NULL));
			c->last_received_packet_id = received_packet_id;
			c->buffer_len = buffer_len;
			PRINTF(2,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Setting timer\n",
					rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time());
			ctimer_set(&c->reassembly_timer, REASSEMBLY_TIMEOUT,
					drop_fragments, c);
		} else {
			PRINTF(2,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Resetting timer\n",
					rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time());
			ctimer_restart(&c->reassembly_timer);
		}

		//		PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Before bitvector_set_received, NULL timer is expired: %u\n",
		//				rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), ctimer_expired(NULL));
		bitvector_set_received(c->bitvector_received_fragments, fragment_nr);
		//		PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : After bitvector_set_received, NULL timer is expired: %u\n",
		//				rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), ctimer_expired(NULL));
		...
		if (bitvector_all_received(c->bitvector_received_fragments,
			...
			PRINTF(2,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Stopping timer\n",
					rimeaddr_node_addr.u8[1],rimeaddr_node_addr.u8[0], clock_time());
			//			PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : After bitvector_all_received and before passing control to recv, NULL timer is expired: %u\n",
			//					rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), ctimer_expired(NULL));
			c->status = IDLE;
			if (c->u->recv != NULL)
				c->u->recv(c, from);
		...
		if (c->status == SENDING) {
			PRINTF(2,
					"[%u.%u:%u] frag_unicast::recv_from_unicast() node was already sending, packet discarded!\n",
					rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time());
		} else if (!rimeaddr_cmp(&c->from, from)) {
			PRINTF(2,
					"[%u.%u:%u] frag_unicast::recv_from_unicast() node was already receiving fragments from node [%u.%u], packet discarded!\n",
					rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), c->from.u8[1], c->from.u8[0]);
			return;
	...
	PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : END\n",
			rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time());
	//	PRINTF(3,"[%u.%u:%u] frag_unicast::recv_from_unicast() : Status: Timer is expired: %u\n",
	//			rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), ctimer_expired(NULL));
...
int frag_unicast_send(struct frag_unicast_conn *c, const rimeaddr_t *receiver) {
	...
	PRINTF(3,"[%u.%u:%u] frag_unicast::frag_unicast_send() : Invoked with receiver [%u.%u], message: ",
			rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(), receiver->u8[1], receiver->u8[0]);
	PRINT_ARR(3, msg, msg_len);
...
//			memcpy(c->buffer, msg, msg_len);
			...
			PRINTF(3,"[%u.%u:%u] frag_unicast::frag_unicast_send() : Number of fragments to be sent: %u\n",
...
//					printf("frag_unicast - sending fragment nr %u\n", fragment_nr);
				...
				PRINTF(2,"[%u.%u:%u] frag_unicast::frag_unicast_send() : Sending with last_sent_packet_id %u, fragment nr %u, num_fragments %u, to [%u.%u], len will be %u\n",
						rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time(),
				...
				PRINTF(3,"[%u.%u:%u] frag_unicast::frag_unicast_send() : dataptr's contents: ",
						rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time());
...
void drop_fragments(void *arg) {
	...
	PRINTF(2,"[%u.%u:%u] frag_unicast::drop_fragments() : Dropping fragments due to expired timer!!\n",
			rimeaddr_node_addr.u8[1], rimeaddr_node_addr.u8[0], clock_time());
