...
#include "scopes-membership-simple.h" /* simple membership for membership tests */
#include "scopes-application.h"       /* application interface */
#include "scopes-repository.h"        /* access to scopes repository */
...
PROCESS_THREAD(example, ev, data)
{
  ...
  while(1) {
    ...
    static uint8_t spec[] = { OPERATOR_AND,
                              PREDICATE_GT, REPOSITORY_VALUE, SENSOR_TEMPERATURE_CELSIUS, UINT8_VALUE, 20,
                              PREDICATE_LT, REPOSITORY_VALUE, SENSOR_LIGHT_TSR_RAW, UINT8_VALUE, 100 };
    ...
    scopes_send(&application, light_scope, application_data, strlen(application_data));
